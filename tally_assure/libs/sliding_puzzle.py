...
out_prefix :Path )->None :
    ...
    row_sums_W [row_sums_W ==0 ]=1 
    p =W /row_sums_W [:,None ]

    a =row_totals [:,None ]*p 
    row_w =1 /np .maximum (row_totals ,1 )**2 
    ...
    for j ,col in enumerate (cols ):
        if j ==pvo_j :
            continue 
        targets [j ]=float (candidate_totals .get (base_name (col ),candidate_totals .get (col ,0 )))
    ...
    while True :
        ...
        frac =np .where (frac <1e-12 ,0 ,frac )
        frac =np .where ((1 -frac )<1e-12 ,0 ,frac )
        nonzero_fracs =frac [frac >0 ]
        delta =float (nonzero_fracs .min ())if nonzero_fracs .size >0 else 1 
        if delta >1 :
            delta =1 
