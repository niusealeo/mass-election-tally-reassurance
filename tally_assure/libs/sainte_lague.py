...
out_path :Path )->None :
    ...
    for c in cols :
        if c =="Party Vote Only":
            continue 
        v =float (candidate_totals .get (base_name (c ),candidate_totals .get (c ,0 )))
        true_row [c ]=v 
        tmp_sum +=v 
    ...
    for c in cols :
        res_row [c ]=float (sum_row .get (c ,0 ))-float (true_row .get (c ,0 ))
